...
def backward_warp_motion(img: torch.Tensor, motion: torch.Tensor) -> torch.Tensor:
    ...
    output = F.grid_sample(img, vgrid, mode='nearest', align_corners=False)
    return output
...
# def optical_flow_to_motion_3d(rgb_flow: torch.Tensor, sensitivity: float = 0.5) -> torch.Tensor:
#     """
#     Returns motion vectors as a [B, 2, D, H, W]
#     with [:, 0, :, :, :] the abscissa and [:, 1, :, :, :] the ordinate.
#     """
#     # flow is: batch x 3-channel x height x width
#     # todo: rgb_to_hsv  is extremely slow (from pytorch_color)
...
def upsample_zero_2d(img: torch.Tensor,
    ...
    # output shape is: [B, C, f[0] * H, f[1] * W]
    # size = None
    ...
    input_size = torch.tensor(img.size(), dtype=torch.int) # [B, C, H, W]
    input_image_size = input_size[2:] # [H, W]
    data_size = input_size[:2] # [B, C]
    if size is None:
    ...
    ##
    # cat([B, C], [f[0]*H, f[0]*W]) = [B, C, f[0]*H, f[0]*W]
    ...
    return output
def upsample_zero_3d(img: torch.Tensor,
                     size: Union[Tuple[int, int], None] = None,
                     scale_factor: Union[Tuple[int, int], List[int], int, None] = None) \
        -> torch.Tensor:
    """
    IMPORTANT: we only support integer scaling factors for now!!
    """
    # input shape is: [B, C, D, H, W]
    # output shape is: [B, C, D, f[0] * H, f[1] * W]
    # size = None
    if size is not None and scale_factor is not None:
        raise ValueError("Should either define both size and scale_factor!")
    if size is None and scale_factor is None:
    ...
    # [H, W]
    input_image_size = input_size[3:]
    # [B, C, D]
    data_size = input_size[:3]
    if size is None:
        # Get the last two dimensions -> height x width
        # compare to given scale factor
        b_ = np.asarray(scale_factor)
